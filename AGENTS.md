...
This project uses **Tailwind CSS v4** with PostCSS. Configuration is CSS-first (no `tailwind.config.js`).

- PostCSS config: `postcss.config.mjs`
- CSS entry point: `src/app/(frontend)/styles.css`

```css
/* Tailwind v4 setup in styles.css */
@import 'tailwindcss';
@plugin 'daisyui';
```

> **Note**: Tailwind v4 does NOT use `tailwind.config.js`. All customization is done via CSS with `@theme` directives.

### DaisyUI v5

**DaisyUI v5** provides themed component classes for Tailwind CSS. Use DaisyUI classes for all UI components in the frontend.
...
<div class="navbar bg-base-100">
  ...
  <div class="navbar-end"><button class="btn">Action</button></div>
</div>
...
```css
/* Custom theme via @plugin options */
@plugin "daisyui" {
...
**@tanstack/react-table v8** for powerful, headless tables. Always combine with DaisyUI table classes for styling.
...
const columns: ColumnDef<MyData>[] = [
  ...
  { accessorKey: 'email', header: 'Email' },
  {
    ...
    header: 'Estado',
    cell: ({ getValue }) => (
      <span className={`badge ${getValue() === 'active' ? 'badge-success' : 'badge-error'}`}>
        {getValue() as string}
      </span>
    ),
  },
...
**@tabler/icons-react** for SVG icons. Import individual icons to keep bundle size small.

```tsx
import { IconUser, IconSettings, IconSearch, IconPlus, IconTrash, IconEdit } from '@tabler/icons-react'
